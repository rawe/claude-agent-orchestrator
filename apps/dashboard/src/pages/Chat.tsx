...
export function Chat() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const isIdle = state.agentStatus === 'idle';
      if (currentSessionId && isIdle) {
        ...
        // No session, or session is terminal — start new session
        const request = {
          parameters: { prompt },
          async_mode: true,
  ...
  // Determine if we can stop (agent is running or idle and we have a session)
  const canStop = (isSessionActive() || state.agentStatus === 'idle') &&
