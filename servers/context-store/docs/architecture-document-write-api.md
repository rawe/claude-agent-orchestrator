...
For future implementation, supporting partial updates via string replacement:
...
Mirrors Claude's Edit tool behavior.

### Offset-Based Partial Writes

Symmetric with existing partial reads:

```
PUT /documents/{document_id}/content?offset=2000
```

Insert or replace content at specific character position.

---

## Implementation Checklist

- [ ] Add `PUT /documents/{document_id}/content` endpoint
- [ ] Modify `POST /documents` to support JSON body without file (placeholder mode)
- [ ] Update database to ensure `checksum` column allows NULL
- [ ] Implement content-type inference from filename
- [ ] Add semantic search re-indexing on content write
- [ ] Create `doc-create` CLI command
- [ ] Create `doc-write` CLI command
- [ ] Add `doc_create` MCP tool
- [ ] Add `doc_write` MCP tool
...
- [Claude Code Write Tool](https://docs.anthropic.com/claude-code) - Inspiration for write semantics
