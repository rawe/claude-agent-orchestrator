...
1. **list_agent_definitions** - Discover available agent blueprints
...
- `agent_definition_name` (optional): Name of agent blueprint to use for this session (optional for generic sessions)
- `project_dir` (optional): Project directory path (must be absolute path). Only set when instructed to set a project dir!
