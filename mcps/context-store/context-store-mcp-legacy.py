...
# ///
"""
Context Store MCP Server - LEGACY IMPLEMENTATION

DEPRECATED: This is the legacy implementation that uses subprocess calls to the
co-located Python CLI commands in plugins/context-store/skills/context-store/commands/.
Each MCP tool spawns a subprocess to execute a doc-* command (e.g., doc-push, doc-read).

Use context-store-mcp.py instead, which uses direct async HTTP calls to the
Context Store server, eliminating subprocess overhead.
...
Usage:
    ...
    uv run context-store-mcp-legacy.py

    # HTTP mode (for network access)
    uv run context-store-mcp-legacy.py --http-mode
    uv run context-store-mcp-legacy.py --http-mode --port 9501
