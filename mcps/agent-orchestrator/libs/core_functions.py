...
async def start_agent_session_impl(
    ...
    try:
        ...
        logger.info(f"Created run {run_id} for session {session_id} (mode={mode.value})")

        if mode != ExecutionMode.SYNC:
            # Async modes: return immediately
            response = {
                ...
                "message": "Agent started in background." + (
                    " Parent will be resumed on completion." if mode == ExecutionMode.ASYNC_CALLBACK
                    else " Use get_agent_session_status to poll for completion."
                ),
            }
            if mode == ExecutionMode.ASYNC_CALLBACK and parent_session_id:
...
async def resume_agent_session_impl(
    ...
    try:
        ...
        logger.info(f"Created resume run {run_id} for session {session_id} (mode={mode.value})")
        ...
        if mode != ExecutionMode.SYNC:
            ...
            response = {
                ...
                "status": "running",
                "mode": mode.value,
                "message": "Agent resumed in background." + (
                    " Parent will be resumed on completion." if mode == ExecutionMode.ASYNC_CALLBACK
                    else " Use get_agent_session_status to poll for completion."
                ),
            }
