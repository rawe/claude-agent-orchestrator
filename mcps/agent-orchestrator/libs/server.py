...
async def start_agent_session(
    ...
    Returns:
      The result/output from the completed agent session, or session info (including session_id) if async_mode=True.
    ...
    """
    # Get HTTP headers for parent session ID extraction (callback support)
    http_headers = get_http_headers()
    return await start_agent_session_impl(
        config, prompt, agent_blueprint_name, project_dir, async_mode, callback, http_headers
    )
...
async def resume_agent_session(
    ...
    # Get HTTP headers for parent session ID extraction (callback support)
    http_headers = get_http_headers()
    return await resume_agent_session_impl(config, session_id, prompt, async_mode, callback, http_headers)
