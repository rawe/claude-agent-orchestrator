...
async def start_agent_session(
    ...
    Session ID is automatically generated by the coordinator (ADR-010).
    Use get_agent_session_status or get_agent_session_result with the returned session_id.

    Execution modes (ADR-003):
      - sync: Wait for child to complete, return result directly (default)
      - async_poll: Return immediately, use get_agent_session_status to poll
      - async_callback: Return immediately, parent will be auto-resumed on child completion

    Returns:
      The result/output from the completed agent session, or session info if mode is async.
    ...
    try:
        ...
        return f"Error: Invalid mode '{mode}'. Must be one of: sync, async_poll, async_callback"

    # Get HTTP headers for parent session ID extraction
    http_headers = get_http_headers()
    return await start_agent_session_impl(
        config, prompt, agent_blueprint_name, project_dir, execution_mode, http_headers
    )
...
async def resume_agent_session(
    session_id: str = Field(
        description="ID of the existing agent session to resume",
        min_length=1,
    ),
    prompt: str = Field(
    ...
    Returns:
      The result/output from the resumed agent session, or session info if mode is async.
    ...
    # Get HTTP headers for parent session ID extraction
    http_headers = get_http_headers()
    return await resume_agent_session_impl(config, session_id, prompt, execution_mode, http_headers)
...
async def get_agent_session_status(
    session_id: str = Field(
        description="ID of the agent session to check status for",
        min_length=1,
    ),
    ...
    """
    return await get_agent_session_status_impl(config, session_id, wait_seconds)


@mcp.tool()
async def get_agent_session_result(
    session_id: str = Field(
        description="ID of the completed agent session to retrieve result for",
        min_length=1,
    ),
) -> str:
    ...
    """
    return await get_agent_session_result_impl(config, session_id)
